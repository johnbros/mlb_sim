...
class ColdStartDataset(Dataset):
    def __init__(self, year, split, config):
        self.year = year if split == 'train' else year + config["validate_shift"]
        self.config = config
    ...
    def _load_data(self):
        ...
        cur.execute("""
            ...
            ORDER BY game_date_time, game_id, inning_num, inning_half, at_bat_num, pitch_num
        """, (self.year,))
        r = cur.fetchall()
        cur_pitcher = None
